...
gcloud run domain-mappings delete --domain=$DOMAIN --region=$REGION --project=$PROJECT_ID --quiet 2>/dev/null || echo "No existing mapping found"
...
gcloud run domain-mappings describe $DOMAIN --region=$REGION --project=$PROJECT_ID --format="value(status.resourceRecords)"
