...
def test_booking_workflow():
    print("🚀 Starting ETG Booking Workflow Integration Test (Future Dates)")
    print("-" * 50)

    # 1. Get Hotel Page (HP) for Conrad LA (Future Dates)
    ...
    rate = rates[0]
    original_hash = rate.get('book_hash') or rate.get('hash') or rate.get('match_hash')
